...
export const getFacebookLoginUrl = async (req, res) => {
  const redirectUri = encodeURIComponent(`${process.env.BASE_URL}/v1/connect/facebook/callback`);
...
export const facebookCallback = async (req, res) => {
  try {
    ...
    const shortLivedRes = await axios.get(
      ...
      `&redirect_uri=${encodeURIComponent(`${process.env.BASE_URL}/v1/connect/facebook/callback`)}` +
